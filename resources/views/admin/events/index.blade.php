...
<div class="p-6 bg-gray-100 min-h-screen">
    ...
    <tr>
        ...
        <th class="p-3 w-32">Status</th> <!-- tambahan kolom status -->
...
<tbody>
    @forelse($events as $event)
        <tr class="border-b hover:bg-gray-50">
           ...
           <td class="p-3">
    <form action="{{ route('events.setStatus', $event) }}" method="POST">
        @csrf
        @method('PATCH')
        <select name="status" onchange="this.form.submit()"
                class="border rounded px-2 py-1">
            <option value="previously" {{ $event->status=='previously' ? 'selected' : '' }}>Previously</option>
            <option value="upcoming" {{ $event->status=='upcoming' ? 'selected' : '' }}>Upcoming</option>
            <option value="now" {{ $event->status=='now' ? 'selected' : '' }}>Now</option>
        </select>
    </form>
</td>
    ...
    @empty
        <tr>
            <td colspan="9" class="p-3 text-center text-gray-500 italic">Belum ada event</td>
        </tr>
    @endforelse
</tbody>
